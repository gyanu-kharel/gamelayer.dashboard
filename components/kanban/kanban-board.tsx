...
export function KanbanBoard() {
  ...
  const handleDragEnd = (event: any) => {
    ...
    setColumns((columns) => {
      ...
      const oldColumn = columns.find((col) => col.tasks.some((task) => task.id === activeTask.id))
      ...
      return newColumns.map((col) => {
        if (col.id === overColumn.id) {
          return {
            ...col,
            tasks: [...col.tasks, { ...activeTask, status: col.id as ColumnType }],
          }
        }
        return col
      })
  ...
  const confirmDeleteColumn = () => {
    if (columnToDelete) {
      setColumns((columns) => columns.filter((col) => col.id !== columnToDelete.id))
  ...
  return (
    <div className="flex flex-col h-screen p-4">
      ...
      <TaskDialog
        ...
        onAssignUser={(userId) => {
          if (selectedTask) {
            const user = userId === "unassigned" ? null : users.find((u) => u.id === userId) || null
      ...
      <AlertDialog open={!!columnToDelete} onOpenChange={() => setColumnToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmDeleteColumn}>Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
